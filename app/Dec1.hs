...
solve :: [Char] -> Int
solve ll = go 0 ll where
  x0 = head ll
  accum acc a = readInt [a] + acc
  go acc [] = acc
  go acc [x1]
    | x0 == x1 = go (accum acc x1) []
    | otherwise = go acc []
  go acc (x1:xrest) =
    let x2 = head xrest
    in
      if x1==x2
      then go (accum acc x1) xrest
      else go acc xrest
...
solve2 ll = 2 * foldr cmpAcc 0 (zip a b)
  where
    nh = round $ fromIntegral (length ll) / 2
    a = take nh ll
    b = drop nh ll
    cmpAcc (x,y) acc
      | x == y = acc + (read [x] :: Int)
